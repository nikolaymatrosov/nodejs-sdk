...
function createBaseServiceAccount_LabelsEntry(): ServiceAccount_LabelsEntry {
  return { $type: "yandex.cloud.iam.v1.ServiceAccount.LabelsEntry", key: "", value: "" };
}

export const ServiceAccount_LabelsEntry = {
  $type: "yandex.cloud.iam.v1.ServiceAccount.LabelsEntry" as const,

  encode(message: ServiceAccount_LabelsEntry, writer: _m0.Writer = _m0.Writer.create()): _m0.Writer {
    if (message.key !== "") {
  ...
  decode(input: _m0.Reader | Uint8Array, length?: number): ServiceAccount_LabelsEntry {
    const reader = input instanceof _m0.Reader ? input : _m0.Reader.create(input);
    let end = length === undefined ? reader.len : reader.pos + length;
    const message = createBaseServiceAccount_LabelsEntry();
    while (reader.pos < end) {
      const tag = reader.uint32();
      switch (tag >>> 3) {
        case 1:
  ...
  fromJSON(object: any): ServiceAccount_LabelsEntry {
    return {
      $type: ServiceAccount_LabelsEntry.$type,
  ...
  create(base?: DeepPartial<ServiceAccount_LabelsEntry>): ServiceAccount_LabelsEntry {
    return ServiceAccount_LabelsEntry.fromPartial(base ?? {});
  },
